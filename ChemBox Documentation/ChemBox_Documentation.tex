...
\item Canvas:\\
The canvas is the area in which the user can draw their molecules. There are a number of essential features that must be included here. The user must be able to draw atoms by clicking on the canvas. Upon selecting an existing atom on the canvas, depending on the chosen action type, the user should either have the option to draw and bond a new atom from the shown potential atoms or bond the atom to one of the other atoms that it could possibly bond to, which will be highlighted. 
